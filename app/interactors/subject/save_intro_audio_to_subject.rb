...
class Subject::SaveIntroAudioToSubjectContext < ActiveInteractor::Context::Base
  attributes :subject, :audio, :audio_type

  validates :subject, :audio, :audio_type, presence: true, on: :calling
end

class Subject::SaveIntroAudioToSubject < ActiveInteractor::Base
  def perform
  ...
  def attach_audio_to_subject
    context[:subject].intro_audio.attach(
