...
class Subject::SaveSummaryTranscriptionContext < ActiveInteractor::Context::Base
  attributes :subject, :transcription

  validates :subject, :transcription, presence: true, on: :calling
end

class Subject::SaveSummaryTranscription < ActiveInteractor::Base
  def perform
    unless context[:subject].update(summary_transcription: preperad_transcription)
